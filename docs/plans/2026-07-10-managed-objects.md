...
Add `insert_sql_file_uploads_and_queries_datalocation`: create a `FILE`
column table, execute `INSERT INTO robots.episodes (...) VALUES (?, ?)` with
`InsertValue::File(FileUpload::from_path(...))`, query the row through
`QueryEngine`, and open the returned location to assert byte equality.
...
`FileUpload` parameter, scalar/`FILE` RecordBatch builder, and direct reader.
