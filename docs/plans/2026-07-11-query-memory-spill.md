...
5. Backport an atomic spill-quota reservation fix for DataFusion 53.1 so a
   rejected write never leaks global disk accounting; cover recovery on the
   same shared runtime.
6. Document sizing, local disk ownership, and the distinction between
...
- `cargo test -p lake-query spill_budget_error_does_not_poison_runtime`
