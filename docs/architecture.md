...
a finite per-tick page budget, stops at end-of-scan without wrapping, and
resumes its process-local cursor on the next tick. Pending records are
...
- The same tick drains multiple append-operation pages under an independent
  finite page budget. It remains serial and cancellation-aware so increasing
  cleanup throughput cannot bypass per-table locks, exact-stage cleanup, or
  starve the later table-maintenance page.
