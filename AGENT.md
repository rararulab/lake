...
* 禁止在 `main` 分支上直接修改代码。
* 任何代码改动都必须先切到独立分支或独立 worktree（`.worktrees/issue-N-<slug>`），再开始编辑。
* 格式化使用 `just fmt`（单 crate，`cargo +nightly fmt --all` 是安全的）；不要手工改格式化配置。
...
* 命名与格式：
  ...
  * Python：遵循 PEP 8；
  * 其他语言遵循对应社区主流风格。
* 在给出较大代码片段时，默认该代码已经过对应语言的自动格式化工具处理（如 `just fmt`、`gofmt`、`black` 等, 一般这种format都会有配置，需要使用justfile中编写好的fmt rule， 如just fmt）。
* 注释：
  * 仅在行为或意图不明显时添加注释；
...
* SQL 引擎：`datafusion`（Catalog/SchemaProvider 接 KV metastore）；对外协议方向是 Arrow Flight SQL，不做 MySQL wire protocol。
* Metadata：KV 只存 version 指针（RocksDB dev / DynamoDB prod，藏在 `MetaStore` trait 后面）；manifest 是不可变文件，先写 manifest 再 CAS 指针。
* 数据格式：Parquet（方向是 Lance，具身智能多模态 blob）。
* 后端项目重点：
  * 高并发读路径靠不可变性 + 缓存，不靠中心存储扛压；
  * 架构不变量见 `CLAUDE.md`，违反需要显式决策。
* 模块组织：当前单 crate；按业务域增长后再拆 workspace，不预先拆分。
