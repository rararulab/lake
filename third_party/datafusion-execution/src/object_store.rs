...
//! ObjectStoreRegistry holds all the object stores at Runtime with a scheme for
//! each store. This allows the user to extend DataFusion with different storage
//! systems such as S3 or HDFS and query data inside these systems.

use std::sync::Arc;

use dashmap::DashMap;
use datafusion_common::{DataFusionError, Result, exec_err, internal_datafusion_err, not_impl_err};
...
///
/// 1. `s3://my_bucket/lineitem/` mapped to the `/lineitem` path on an AWS S3
///    object store bound to `my_bucket`
///
/// 2. `s3://my_other_bucket/lineitem/` mapped to the (same) `/lineitem` path on
///    a *different* AWS S3 object store bound to `my_other_bucket`
///
...
/// ```
///
/// In this particular case, the url `s3://my_bucket/lineitem/` will be provided
/// to [`ObjectStoreRegistry::get_store`] and one of three things will happen:
///
/// - If an [`ObjectStore`] has been registered with
///   [`ObjectStoreRegistry::register_store`] with `s3://my_bucket`, that
///   [`ObjectStore`] will be returned
///
/// - If an AWS S3 object store can be ad-hoc discovered by the url
///   `s3://my_bucket/lineitem/`, this object store will be registered with key
///   `s3://my_bucket` and returned.
///
/// - Otherwise an error will be returned, indicating that no suitable
///   [`ObjectStore`] could be found
///
/// This allows for two different use-cases:
///
/// 1. Systems where object store buckets are explicitly created using DDL, can
///    register these buckets using [`ObjectStoreRegistry::register_store`]
///
/// 2. Systems relying on ad-hoc discovery, without corresponding DDL, can
///    create [`ObjectStore`] lazily by providing a custom implementation of
///    [`ObjectStoreRegistry`]
///
...
pub trait ObjectStoreRegistry: Send + Sync + std::fmt::Debug + 'static {
    ...
    /// the deregistered store if it existed.
    ...
    /// executed depending on the `url` and [`ObjectStoreRegistry`]
    /// implementation. An [`ObjectStore`] may be lazily created and
    /// registered.
...
pub struct DefaultObjectStoreRegistry {
    /// A map from scheme to object store that serve list / read operations for
    /// the store
...
impl ObjectStoreRegistry for DefaultObjectStoreRegistry {
    ...
    fn deregister_store(&self, url: &Url) -> Result<Arc<dyn ObjectStore>> {
        let s = get_url_key(url);
        let (_, object_store) = self.object_stores.remove(&s).ok_or_else(|| {
            internal_datafusion_err!(
                "Failed to deregister object store. No suitable object store found for {url}. See \
...
mod tests {
    ...
    fn test_object_store_url() {
        ...
        let err = ObjectStoreUrl::parse("s3://username:password@host:123/foo").unwrap_err();
        assert_eq!(
            err.strip_backtrace(),
            "Execution error: ObjectStoreUrl must only contain scheme and authority, got: /foo"
        );
