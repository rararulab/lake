...
use datafusion_common::{
    DataFusionError, Result, config_err, human_readable_size, resources_datafusion_err,
    resources_err,
};
use log::debug;
...
impl DiskManagerBuilder {
    pub fn set_mode(&mut self, mode: DiskManagerMode) { self.mode = mode; }
...
impl DiskManagerConfig {
    ...
    pub fn new_existing(existing: Arc<DiskManager>) -> Self { Self::Existing(existing) }

    /// Create temporary files in the specified directories
    pub fn new_specified(paths: Vec<PathBuf>) -> Self { Self::NewSpecified(paths) }
}
...
impl DiskManager {
    ...
    pub fn builder() -> DiskManagerBuilder { DiskManagerBuilder::default() }
    ...
    pub fn with_max_temp_directory_size(mut self, max_temp_directory_size: u64) -> Result<Self> {
        self.set_max_temp_directory_size(max_temp_directory_size)?;
        Ok(self)
    }

    pub fn used_disk_space(&self) -> u64 { self.used_disk_space.load(Ordering::Relaxed) }

    /// Returns the maximum temporary directory size in bytes
    pub fn max_temp_directory_size(&self) -> u64 { self.max_temp_directory_size }
...
///
/// # Note
/// After any modification to the underlying file (e.g., writing data to it),
/// the caller must invoke [`Self::update_disk_usage`] to update the global disk
/// usage counter. This ensures the disk manager can properly enforce usage
/// limits configured by [`DiskManager::with_max_temp_directory_size`].
///
/// This type is Clone-able, allowing multiple references to the same underlying
/// file. The file is deleted only when the last reference is dropped.
///
/// The parent temporary directory is also kept alive as long as any reference
/// to this file exists, preventing premature cleanup of the directory.
///
...
pub struct RefCountedTempFile {
    ...
    /// ensure it is not cleaned up prior to the NamedTempFile
    ...
    /// disk usage tracking, preventing incorrect accounting when clones are
    /// dropped.
...
impl RefCountedTempFile {
    pub fn path(&self) -> &Path { self.tempfile.path() }

    pub fn inner(&self) -> &NamedTempFile { self.tempfile.as_ref() }
    ...
    pub fn update_disk_usage(&mut self) -> Result<()> {
        ...
        if new_disk_usage > old_disk_usage {
            let growth = new_disk_usage - old_disk_usage;
            let reserved = self.disk_manager.used_disk_space.fetch_update(
                Ordering::Relaxed,
                Ordering::Relaxed,
                |current| {
                    current
                        .checked_add(growth)
                        .filter(|next| *next <= self.disk_manager.max_temp_directory_size)
                },
            );
            if reserved.is_err() {
                return resources_err!(
                    "The used disk space during the spilling process has exceeded the allowable \
                     limit of {}. Try increasing the `max_temp_directory_size` in the disk \
            ...
            self.disk_manager
                .used_disk_space
                .fetch_sub(old_disk_usage - new_disk_usage, Ordering::Relaxed);
        }

        // The global reservation succeeded, so Drop can now release this exact size.
    ...
    pub fn current_disk_usage(&self) -> u64 { self.current_file_disk_usage.load(Ordering::Relaxed) }
}
...
mod tests {
    ...
    fn assert_path_in_dirs<'a>(file_path: &'a Path, dirs: impl Iterator<Item = &'a Path>) {
    ...
    fn test_disk_usage_with_clones() -> Result<()> {
        ...
        // Global disk usage should still be the same (not multiplied by number of
        // clones)
        assert_eq!(dm.used_disk_space(), usage_after_write);
    ...
    #[test]
    fn test_disk_limit_rejection_does_not_leak_usage() -> Result<()> {
        use std::io::Write;

        let dm = Arc::new(
            DiskManagerBuilder::default()
                .with_max_temp_directory_size(5)
                .build()?,
        );
        let mut rejected = dm.create_tmp_file("over budget")?;
        rejected.inner().as_file().write_all(b"123456")?;
        assert!(rejected.update_disk_usage().is_err());
        assert_eq!(rejected.current_disk_usage(), 0);
        assert_eq!(dm.used_disk_space(), 0);
        drop(rejected);
        assert_eq!(dm.used_disk_space(), 0);

        let mut accepted = dm.create_tmp_file("after rejection")?;
        accepted.inner().as_file().write_all(b"12345")?;
        accepted.update_disk_usage()?;
        assert_eq!(dm.used_disk_space(), 5);
        drop(accepted);
        assert_eq!(dm.used_disk_space(), 0);
        Ok(())
    }
